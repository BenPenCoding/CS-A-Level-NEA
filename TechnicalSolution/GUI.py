...
def beginTutorial(board, turn, numMoves):		#Begins a chess game against a bot
	...
	pygame.display.set_caption('Build Your Chess - Offline tutorial')
	...
	while running:
		...
		#Displays the best move to make
		...
		[bestPiece, bestLocation] = game.getBestMove(2, "White", "White")[0]

		bestPieceType = bestPiece.getPieceType()
		bestPieceRank, bestPieceFile = bestPiece.getBoardCoords()

		bestPieceRank = str(8 - bestPieceRank		)
		bestPieceFile = locationTypeDict[bestPieceFile]

		[bestLocationRank, bestLocationFile] = bestLocation

		bestLocationRank = str(8 - bestLocationRank)		
		bestLocationFile = locationTypeDict[bestLocationFile]

		screen.blit(buttonFont.render(f"Best move: {bestPieceType} at {bestPieceFile+bestPieceRank} to {bestLocationFile+bestLocationRank}", True, (0,0,0)), (100, 500))	
